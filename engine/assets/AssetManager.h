...
public:
	...
	static std::shared_ptr<T> Get(AssetId id)
	{
		...
		{
			// Loaded
			return std::static_pointer_cast<T, Asset>(m_loadedAssets[id]);
		}
		// Not loaded
		...
		m_loadedAssets[index[id].id] = asset;
		return asset;
	}
...
private:
	...
	static std::unordered_map<AssetId, std::shared_ptr<Asset>> m_loadedAssets;
};
} // namespace nimo
