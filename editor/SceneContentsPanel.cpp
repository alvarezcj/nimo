...
{
    static ImGuiTreeNodeFlags base_flags = ImGuiTreeNodeFlags_OpenOnArrow | ImGuiTreeNodeFlags_SpanAvailWidth | ImGuiTreeNodeFlags_SpanFullWidth;
    static nimo::GUID selectedEntity;
    for(auto [name, scene] : m_editor->loadedScenes)
    {
        ...
        if(ImGui::CollapsingHeader((name + "##" + scene.second->id.str()).c_str()))
        {
            ...
            scene.second->ForEachEntity([&](nimo::Entity& ent)
            {
                ...
                bool open = ImGui::TreeNodeEx((entityName + "##" + scene.second->id.str()).c_str(), node_flags);
                if (ImGui::IsItemClicked() && !ImGui::IsItemToggledOpen())
                {
                    selectedEntity = ent.GetComponent<nimo::IDComponent>().Id;
                    NIMO_DEBUG("Selected entity: {}", ent.GetComponent<nimo::LabelComponent>().Label);
                }
                if(open)
                {
                    ImGui::TreePop();
